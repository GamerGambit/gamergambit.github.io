...
In this article we are going to implement a K2 node that does a HTTP Get request.
...
## Latent vs Async
Aync is used to define a task that executes independently from the main thread and not block it. While the latent blueprint nodes don't block, they are not asynchronous as they are ticked with the game thread in the background.

While the title of this article might be slightly misleading, Epic called it `UK2Node_BaseAsyncTask` so thats what I call them. For the purposes of this article, the 2 terms are interchangable.

# Subclassing UK2Node_BaseAsyncTask
As mentioned above, `UK2Node_BaseAsyncTask` is probably the easiest way to make a K2 node. Internally it handles pin allocation, node expansion, gives us the clock face icon denoting its latent, and even puts our node in the proper category in the actions menu, and actually implements `GetMenuActions` for us.

The only 2 things we need in order to make an asyc node is to make a "proxy" class which is created by the K2 node and actually does runtime stuff, and subclass `UK2Node_BaseAsyncTask` so we can pass in our proxy class.

## Proxy Class
The proxy class is a `UObject` that holds dynamic multicast delegates since thats the entire purpose of this node plus whatever other data and functions your node needs to actually do anything.
...
The source for this is just the implementation of `CreateProxyObjectForHTTPRequest`. The name of this function is *sort* of important since it's what `UK2Node_BaseAsynTask` uses to name our node which can be overwritten using the [DisplayName meta](https://benui.ca/unreal/ufunction/#displayname) attribute. We use the `RF_StrongRefOnFrame` flag to stop shiny new proxy object getting garbage collected since we don't store our proxy object anywhere.
...
Just like that we have a nonfunctional async/latent node which was significantly easier to get in the graph than a regular K2 node.

![HTTP node](/assets/img/httpnode.png)

Something doesn't look quite right. In our `UHttpReqProxy` class we have a data member: `FString URL`. There is no pins on the node for this input, so lets fix that.

## Data In, Data Out
In order to get data in and out of our node we need to make full use of our factory function `CreateProxyObjectForHTTPRequest`. `UK2Node_BaseAsyncTask` uses `UK2Node_CallFunction` node to get pins on our factory function and allocate them on the node. To get our URL pin showing on the node we just need to add it as an input to the factory function.
...
We can extend this behavior to our delegate too. If we specify that our delegate has parameters they will be allocated pins on our node automatically. If we have multiple delegates with the same signature, the data pins will be shared between the delegate's execution pins.
...
# Expansion
Lets compile a Blueprint with our node and see what gets created.
...
First a function call to our factory function is made then for each of our delegates a custom event is created and bound to the delegate.
...
* `HideThen` will not create a `then` pin on the node meaning the only output execution pins will be for delegates.

# HTTP Request
I almost forgot to actually implement a get request for our node so lets get that sorted. We need to add the `HTTP` module to our `{Project}.build.cs` and change our factory function to this:
...
I was planning a 4th article which would go over making a custom async node from scratch since there are some cases where `UK2Node_BaseAsyncTask` won't work (e.g. if you have delegates that you don't want execution pins for) and having to put all your node inputs into a single function is quite tedious, however I don't think that has much value and if you *really* need this, studying `UK2Node_BaseAsyncTask` will give you all of the answers you need.

If you made it this far with your sanity in tact congratulations, I hope you learnt something about K2 nodes.
