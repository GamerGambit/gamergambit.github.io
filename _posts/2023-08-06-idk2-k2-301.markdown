...
The name of the factory function is what `UK2Node_BaseAsynTask` uses to name our node and can be overwritten using the [DisplayName meta attribute](https://benui.ca/unreal/ufunction/#displayname).
...
There are some cases where `UK2Node_BaseAsyncTask` won't work, such as delegates which shouldn't have execution pins. Studying the UK2Node_BaseAsyncTask source should give you all the answers you need.
